 ...
 * src/scoring/solar.ts (frontend, direct-source scoring path). The two must
 * stay logically identical -- see that file's header comment for the
 * reverse pointer. Keep both in sync by hand if this math ever changes
 * (same hand-sync convention already used for the dress-advice thresholds
 * in scoring.ts / src/scoring/score.ts).
 */
