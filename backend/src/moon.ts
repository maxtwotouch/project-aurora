 ...
 * src/scoring/moon.ts (frontend, direct-source scoring path). The two must
 * stay logically identical -- see that file's header comment for the
 * reverse pointer. Keep both in sync by hand (same hand-sync convention
 * already used for solar.ts / season.ts).
 *
